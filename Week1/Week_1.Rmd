---
title: An R Markdown document converted from "Week1.ipynb"
output: html_document
---

```{r}
Sys.setenv(LD_PRELOAD="/usr/lib/x86_64-linux-gnu/libgfortran.so.3")
packages_to_be_loaded=c("corrplot","dummies","forecast","gains","reshape","mlr")
...
```{r}
# instead of using leaps, we will use mlr -- this is only necessary to get this to work on azure. Moreover, i
dummy_remodel=dummy("REMODEL",housing.index)
housing.search=cbind(housing.index,dummy_remodel)
housing.search=housing.search[,c(-13,-14)]


ctrl=makeFeatSelControlExhaustive(maxit = 1)
rdesc=makeFixedHoldoutInstance(1:4641, 4642:5802, 5802)
tsk=makeRegrTask(data = housing.search, target = "TOTAL.VALUE")
sfeats = selectFeatures(learner='regr.lm',task=tsk,resampling=rdesc,control=ctrl,show.info = F)
```
...
```{r}
selected_vars=sfeats$x
```

The MLR based exhaustive subset regression chooses 11 variables: `LOT.SQFT`, `GROSS.AREA`, `LIVING.AREA`, `FLOORS`, `BEDROOMS`, `FULL.BATH`, `HALF.BATH`, `KITCHEN`, `FIREPLACE`, `REMODELOld`, and `REMODELRecent`. The basis for the selection is the mean squared error (MSE) calculated over the validation data. If you compare this result to the result that is obtained through leaps, you will notice some differences. These are due to differences in the criteria used to determine the best subset (the leaps code above uses in-sample adjusted R^2, and is likely to overfit -- more on this in the coming weeks). 

```{r}
housing.lm.exhaust <- lm(TOTAL.VALUE~. , data=train[,c(selected_vars,'TOTAL.VALUE')])
...
Is it "cheating" if I use the RMSE (or any other error metric) to *go back* and choose an alternative model in Step #9? Yes! It's cheating (this is compounded in the case of exhaustive search because we are using the validation set to select the model in the first place). If you do choose to use the error metrics from the validation set to revise your model selection, you will need to use a third partition (i.e. the test set) to get true estimates of the error metrics.   
